...
do	-- (separate block to garbage-collect drawing objects)
	...
	local logo_ctx = g2d.create_context(logo_image)
	...
	for i=1, 3 do
		...
		if i<3 then
			g2d.image_convolution(logo_image, blur_kernel)
		end
	end
end
...
function GetFrame(frame, frame_i)
	...
	local frame_ctx = g2d.create_context(frame)
	-- Draw logo onto frame
	frame_ctx:set_source(g2d.create_source_image(logo_image))
	frame_ctx:paint()
end