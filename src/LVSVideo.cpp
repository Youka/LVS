...
LVSVideo::LVSVideo(const char* script, int width, int height, bool has_alpha, double fps, int frames, const char* data_string) : LVSMediaBase(data_string){
	...
	lua_pushboolean(this->L, has_alpha); lua_setglobal(this->L, "VIDEO_HAS_ALPHA");
...
void LVSVideo::Render(CairoImage* image, int frame_number){
	...
	if(lua_isfunction(this->L, -1)){
		...
		*lua_createuserdata<cairo_surface_t*>(L, "g2d image") = cairo_image_surface_create_for_data(const_cast<unsigned char*>(image->data), image->has_alpha ? CAIRO_FORMAT_ARGB32 : CAIRO_FORMAT_RGB24, image->width, image->height, image->stride);	// Image object of 'g2d' library
